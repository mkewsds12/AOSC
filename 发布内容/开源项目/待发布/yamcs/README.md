# Yamcs · GitLink 社区镜像组织

> 来源：https://github.com/yamcs
> 定位：在 GitLink 建立「Yamcs」的国内社区镜像组织，批量镜像其开源仓库
> 状态：待执行
...
| 表单项 | 填写内容 |
|--------|---------|
| 组织账号 | `yamcs`（若提示已被占用，改用 `yamcs-mirror`） |
| 组织名称 | `Yamcs（社区镜像）` |
| 组织描述 | 非官方社区镜像，仅收录 Yamcs 公开发布的开源项目；版权归原作者与来源组织（https://github.com/yamcs）；仓库以只读镜像方式同步，每 8 小时更新一次 |
| 所在地区 | 比利时 |
...
|-----------|---------|--------|-----------|
| Yamcs | [yamcs.md](yamcs.md) | ⚠️ 未检测到 LICENSE 文件 | ⚠️ 无 LICENSE 只收链接 |
...
- yamcs 主仓库未检测到 LICENSE 文件，README 突出「只能收链接」；Yamcs 是专区测运控招牌，建议优先联系官方入驻或许可
