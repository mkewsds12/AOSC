...
> 工作流：**建镜像组织 → 组织内逐个镜像导入仓库 → 挂到专区分类 → 归档到已发布**
> 数据基础：《调研/开源项目调研.md》、2026-08-10 许可证核查（结果在各项目 md 末尾）
> 状态：本目录 = 待发布（待执行）；执行完成后将组织文件夹移入 `../已发布/`

---

## 一、目录结构

每个来源组织一个文件夹（本目录为「待发布」区，执行后移入已发布）：

```text
待发布/
├── README.md            # 本文件（组织镜像总览）
├── 生态入口/            # topic 聚合页 / 官网 / 未公开项目（非镜像，仅收录线索）
...
6. **归档**：该组织全部项目上架完成后，将整个组织文件夹移入 `../已发布/`
