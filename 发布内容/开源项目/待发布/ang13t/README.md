# ANG13T · GitLink 社区镜像组织

> 来源：https://github.com/ANG13T
...
| 表单项 | 填写内容 |
|--------|---------|
| 组织账号 | `ang13t`（被占用则 `ang13t-mirror`） |
| 组织名称 | `ANG13T（社区镜像）` |
| 组织描述 | 非官方社区镜像，仅收录 ANG13T 公开发布的开源项目；版权归原作者（https://github.com/ANG13T）；仓库以只读镜像方式同步，每 8 小时更新一次 |
| 所在地区 | 可留空或按项目背景填写 |
...
|-----------|---------|--------|-----------|
| SatIntel | satintel.md | LICENSE 文件为空（0 字节），无法确认 | ❓ 需人工核查 |
...
- 无 LICENSE 只收链接不镜像
- SatIntel 的 LICENSE 文件存在但为空（GitHub API 识别为 NOASSERTION/Other）：需人工核查许可类型后再决定是否镜像；确认前专区可先只收录官方链接
